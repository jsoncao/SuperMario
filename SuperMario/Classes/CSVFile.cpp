...
bool CSVFile::Open(bool bIsRead, const char* strFilename)
{
	...
	std::string	strFullFileName = CCFileUtils::sharedFileUtils()->fullPathForFilename(strFilename);
...
int replace(std::string& src_str, const std::string& old_str, const std::string& new_str)
{
	...
	unsigned int	pos	= 0;
	while((pos=(unsigned int)src_str.find(old_str,pos))!=std::string::npos)
	{
